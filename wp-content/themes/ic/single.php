...
<div class="fullpic">
	...
	<div class="slide-header">
		<a class="button" onclick="ga('send', 'event', 'Reserve', 'Reservation-button-banner', 'Reserve Now');" href="<?php if(get_post_meta ($post->ID, 'cebo_booklink', true)) { echo get_post_meta ($post->ID, 'cebo_booklink', true); } else { echo get_option('cebo_genbooklink'); } ?>"><?php _e('RESERVE NOW', 'cebolang'); ?></a>
	</div>
...
if( count( $children ) != 0 ) { ?>
  ...
  $parent = $ancestors[0]; query_posts(
			...
			<div class="sidebar fr">
				...
				<a class="button" href="<?php if(get_post_meta ($post->ID, 'cebo_booklink', true)) { echo get_post_meta ($post->ID, 'cebo_booklink', true); } else { echo get_option('cebo_genbooklink'); } ?>" onclick="_gaq.push(['_link', this.href]);ga('send', 'event', 'Booking', 'Reserve', '<?php echo get_the_title(); ?>');return false;"><?php _e('RESERVE NOW', 'cebolang'); ?></a>
