...
$meta_email_signup = array(
	'id' => 'email-signup-options',
	...
	'fields' => array(
 		...
 		// array( 
   //            "name" => "Signup content",
	  //         "desc" => "Type in the content text above the form",
	  //         "id" => $prefix."_signup_text",
	  //         "type" => "text",
	  //         "std" => ""
   //            )
 		// ,
 		array( 
	          ...
	          "desc" => "Type in the text inside the email field",
	          "id" => $prefix."_signup_placeholder",
	          "type" => "text",
	          "std" => ""
              )
 		,
 		array( 
              "name" => "Button Text",
	          "desc" => "Type in the text for the signup button",
	          "id" => $prefix."_signup_buttontext",
	          "type" => "text",
	          "std" => ""
              )
 		,
		...
		array( 
			"name" => "Display when the user scrolls down a third of the page",
			"id" => $prefix."_signup_box_scroll",
			"type" => "checkbox",
			"std" => ""
		),

		array( 
			"name" => "Display when the user stays on the page for 5-10 seconds",
			"id" => $prefix."_signup_box_pageview",
			"type" => "checkbox",
			"std" => ""
		),

		array( 
			"name" => "Display when the user visits the blog twice",
			"id" => $prefix."_signup_box_visit",
			"type" => "checkbox",
			"std" => ""
		),
...
add_action('admin_menu', 'signup_add_boxer');
// Add meta boxer
function signup_add_boxer() {
	global $meta_email_signup;
	foreach ( array( 'email-signup-form' ) as $page )
	add_meta_box($meta_email_signup['id'], $meta_email_signup['title'], 'signup_show_boxer', $page, $meta_email_signup['context'], 			$meta_email_signup['priority']);
}
// Callback function to show fields in meta boxer
function signup_show_boxer() {
	global $meta_email_signup, $post;
...
jQuery(document).ready(function() {
	...
	echo '<input type="hidden" name="signup_meta_boxer_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
	echo '<table class="form-table">';
	foreach ($meta_email_signup['fields'] as $field) {
...
add_action('save_post', 'signup_save_data');
// Save data from meta boxer
function signup_save_data($post_id) {
	global $meta_email_signup;	
	// verify nonce
	if (!wp_verify_nonce($_POST['signup_meta_boxer_nonce'], basename(__FILE__))) {
		return $post_id;
	}
	// check autosave
